    ...
    public partial class WebForm3 : System.Web.UI.Page
    {
        ...
        protected void Dodaj_Nowy_Towar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (nowy_towar_cena.Text == "" || nazwa_towaru == "" || nowy_towar_ilosc.Text == "")
                {
                    lblError.Text = "nieprawidłowe dane";
                }
                else
                {
                    ...
                    con.Open();
                    SqlCommand cmd = new SqlCommand("Select count(*) from towar where Nazwa = '" + nazwa_towaru + "'", con);
                    string getValue = cmd.ExecuteScalar().ToString();

                    if (getValue != "0")
                    {
                        lblError.Text = "Taki towar już istnieje";
                    }
                    else
                    {
